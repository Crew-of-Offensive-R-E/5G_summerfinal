...
def _unit_in_use(unit):
    """systemd 유닛이 실행 중이거나 부팅 시 활성화되어 있는지 확인한다."""
...
def _is_finger_inetd_line(line):
    stripped = line.strip()
    ...
    return service == "finger" or bool(re.search(r"\bfingerd\b", stripped, re.I))
...
def _inetd_issues():
    ...
    return [
        f"{INETD_PATH}:{number}: Finger 서비스 활성 설정"
        for number, line in enumerate(text.splitlines(), start=1)
        if _is_finger_inetd_line(line)
    ]
...
def _xinetd_vulnerable_files():
    ...
    for name in names:
        ...
        related = name.lower() in {"finger", "fingerd"}
        disable_value = None
        ...
        for line in text.splitlines():
            ...
            if re.match(r"^\s*service\s+finger\b", line, re.I):
                related = True

            match = re.match(r"^\s*disable\s*=\s*(yes|no)\b", line, re.I)
            if match:
...
def _issues():
    ...
    if pgrep_any("fingerd", "in.fingerd", "cfingerd"):
        issues.append("fingerd/cfingerd 프로세스 활성")
    if is_listening_on_port(79):
        issues.append("TCP/UDP 79번 포트 리스닝")
    issues.extend(_inetd_issues())

    vulnerable_files, errors = _xinetd_vulnerable_files()
    issues.extend(f"{path}: disable=yes 미설정" for path in vulnerable_files)
    issues.extend(errors)
    return issues


def _disable_inetd(backups):
    issues = _inetd_issues()
    if not issues or not os.path.isfile(INETD_PATH):
        return []
    ...
    for line in lines:
        if _is_finger_inetd_line(line):
            output.append(f"# U-34 disabled: {line}")
            changed = True
        else:
            output.append(line)
...
def _disable_xinetd_file(path, backups):
    ...
    for line in lines:
        ...
        if re.match(r"^\s*disable\s*=", line, re.I):
    ...
    if not write_text(path, "".join(output)):
        return [f"{path}: 쓰기 실패"]
    return []
...
def fix(dry_run=False):
    ...
    if dry_run:
        return result(
            ...
            TITLE,
            FIXED,
            f"dry-run: Finger 서비스 비활성화 예정 — {summarize(before)}",
        )
    ...
    vulnerable_files, scan_errors = _xinetd_vulnerable_files()
    ...
    if errors or remaining:
        ...
        detail = []
        if errors:
            detail.append(f"오류: {summarize(errors)}")
        if remaining:
            detail.append(f"남은 취약 설정: {summarize(remaining)}")
        if restore_errors:
            detail.append(f"원복 오류: {summarize(restore_errors)}")
        elif backups:
            detail.append("변경 파일/메타데이터 원복 완료")
        if backups:
            detail.append(f"백업: {summarize(backups)}")
        return result(CODE, TITLE, FAILED, " | ".join(detail))

    backup_detail = summarize(backups) if backups else "설정 파일 변경 없음"
    return result(
        CODE,
        TITLE,
        FIXED,
        f"Finger 서비스 중지·자동 시작 해제 및 inetd/xinetd 비활성화 완료 | 백업: {backup_detail}",
    )
