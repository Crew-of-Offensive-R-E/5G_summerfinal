"""U-22 /etc/services 파일 소유자 및 권한 설정 조치."""
...
def fix(dry_run=False):
    state = _state()
    if state is None:
        return result(CODE, TITLE, FAILED, f"{TARGET} 파일을 확인할 수 없음")
    info, mode = state
    if info.st_uid == 0 and mode <= 0o644:
        return None

    if dry_run:
        return result(CODE, TITLE, FIXED, f"dry-run: {TARGET} 소유자 root, 권한 0644 적용 예정")
    ...
    if not set_file_owner(TARGET, "root") or not set_file_mode(TARGET, 0o644):
        try:
    ...
    verify = _state()
    if verify is None or verify[0].st_uid != 0 or verify[1] > 0o644:
        try:
            os.chown(TARGET, old_uid, old_gid)
            os.chmod(TARGET, old_mode)
        except OSError:
    ...
    return result(CODE, TITLE, FIXED, f"소유자 root, 권한 0644 적용; 백업: {backup}")
