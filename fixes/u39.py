...
PROCESS_MARKER = "nfsd/rpcbind (pgrep)"


def _get_active():
    active = [service for service in NFS_SERVICES if systemctl_is_active(service)]
...
def fix(dry_run=False):
    ...
    try:
        required = require_bool(policy_for(CODE), "nfs_required")
    ...
    if required:
        return result(
            ...
            TITLE,
            MANUAL,
            "정책상 NFS 사용 서버이므로 자동 중지하지 않음: " + summarize(before),
        )
    ...
    if dry_run:
        return result(
            ...
            TITLE,
            FIXED,
            "dry-run: 정책상 불필요한 NFS 서비스 중지·비활성화 예정 — "
            + summarize(before),
        )
    ...
    return result(
        ...
        TITLE,
        FIXED,
        "Open5GS 단일 서버 정책에 따라 NFS 서비스 중지·비활성화 완료",
    )