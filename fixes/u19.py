"""U-19 /etc/hosts 파일 소유자 및 권한 설정 조치."""
...
def fix(dry_run=False):
    ...
    info, mode = state
    if info.st_uid == 0 and mode <= TARGET_MODE and not (mode & 0o022):
        return None
    ...
    if not set_file_owner(TARGET, "root") or not set_file_mode(TARGET, TARGET_MODE):
        try:
    ...
    verify = _state()
    if verify is None or verify[0].st_uid != 0 or verify[1] != TARGET_MODE:
        try:
            os.chown(TARGET, old_uid, old_gid)
            os.chmod(TARGET, old_mode)
        except OSError:
